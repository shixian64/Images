...
空库首次注册会自动创建管理员账号，不再需要管理员初始化令牌。为兼容旧部署，如果数据库里已有普通用户但没有活跃管理员，仍可设置 `ADMIN_BOOTSTRAP_TOKEN` 并在注册页填写该令牌来初始化管理员。
...
- `REGISTRATION_MODE=closed`：关闭公开注册；空库首个注册账号仍可自动成为 admin。
...
| 路径 | 说明 |
| --- | --- |
| `POST /api/auth/register` | 注册；空库首个账号自动成为管理员 |
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `ADMIN_BOOTSTRAP_TOKEN` | 空 | 可选旧部署兼容令牌；已有普通用户但没有活跃管理员时可用它初始化管理员。 |
