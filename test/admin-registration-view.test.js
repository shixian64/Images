...
test('admin registration view uses locale messages for summary and tables', () => {
  setLocale('en-US');
  assert.equal(formatRegistrationInviteExpiry({}), 'Never expires');
  assert.equal(registrationModeLabel({ allowPublicRegistration: true, allowInviteRegistration: true }), 'Open registration + invite codes');
  assert.equal(registrationModeLabel({ allowPublicRegistration: true }), 'Open registration');
  assert.equal(registrationModeLabel({ allowInviteRegistration: true }), 'Invite-only registration');
  assert.equal(registrationModeLabel({}), 'Registration closed');
  assert.deepEqual(registrationInviteStatus({ remainingUses: 1 }), { className: 'ok', label: 'Available' });
  assert.equal(
    registrationRedemptionUserLabel({ username: 'Alice', email: 'a@example.test', userDeleted: true }),
    'Alice · a@example.test · User deleted'
  );
  assert.match(registrationInviteUsersHtml('', [], 0), /Unused/);
  assert.match(registrationInviteUsersHtml('', [], 1), /Redemption records cleaned/);
  ...
  const summary = registrationSummaryHtml({
    ...
    invites: [{ active: true, remainingUses: 2 }],
    redemptions: [{ code: 'a' }]
  });
  assert.match(summary, /Invite-only registration/);
  assert.match(summary, /Invite registration: Allowed/);
  assert.match(summary, /Default uses: 5/);
  assert.match(summary, /Available invite codes: 1 \/ 2 remaining/);
  assert.match(summary, /Currently sourced from environment variables/);
  assert.match(registrationInvitesHtml(null), /Waiting for registration settings to load/);
  assert.match(registrationInvitesHtml({ invites: [] }), /No invite codes generated by the UI yet/);
  assert.match(registrationRedemptionsHtml({ redemptions: [] }), /No redemption records/);
  assert.match(registrationErrorHtml('boom'), /Load failed: boom/);

  const table = registrationInvitesHtml({
    invites: [{ code: 'abc', displayCode: 'abc', remainingUses: 1 }],
    redemptions: []
  });
  assert.match(table, /Invite \/ redemption codes/);
  assert.match(table, /Used \/ total/);
  assert.match(table, />Disable<\/button>/);

  const redemptions = registrationRedemptionsHtml({
    redemptions: [{ code: 'abc', email: 'u@example.test', userId: 'u1', usedAt: 'bad' }]
  });
  assert.match(redemptions, /Redemption records/);
  assert.match(redemptions, /Registered user/);
  assert.match(redemptions, /Redeemed at/);
