...
| 69 | 部分闭环 | 审计日志 metadata 已统一脱敏、裁剪字符串 / 深度 / 数组 / 对象规模，并限制落库 JSON 体积；client logs 也已有接入预算；generation job 的 result / progress 已增加落库 JSON 预算。prompt square 等跨响应预算仍需继续梳理。 |
