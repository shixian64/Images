...
- **系统默认接口**：管理员在接口管理中配置，普通用户默认继承；实现位于 `services/interface-defaults.js`，配置通过 `services/db.js:systemSettings` 存入 `generated/app.db`。
...
- `generated/`、`.env`、数据库、WAL、生成图片不得提交到 Git。
...
- 日志、截图、Issue、PR 描述不得包含 API Key、Cookie、数据库片段或用户图片。
