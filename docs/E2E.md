...
项目提供一个无第三方依赖的真实浏览器冒烟脚本，用 Chrome / Edge / Chromium 的 DevTools Protocol 打开已运行的 Image Studio 页面，验证登录页真实渲染、表单存在、注册表单默认隐藏，以及入口页没有内联脚本。
...
这不是完整视觉回归；后续可在同一 CDP 基础上扩展登录态、主要 tab 切换、截图基线和差异阈值。
