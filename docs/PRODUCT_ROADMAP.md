# Image Studio · 未来路线图与商业化设计

> 阅读口径：本文只描述未来 SaaS / 商业化演进方向，不等同于当前自托管版本已经具备的能力。
> 当前已落地能力、部署边界和安全边界见 [`PRODUCT_CURRENT.md`](PRODUCT_CURRENT.md)。
...
- **账号与权限**：注册、登录、session、普通用户 / 管理员、本地开发首个注册自动管理员、生产首个管理员 bootstrap token 保护、旧管理员初始化令牌兼容、关闭 / 邀请 / 公开注册模式。
- **管理后台**：用户管理、额度管理、生成队列、客户端日志、系统默认接口、图库管理。
- **生成链路**：异步入队提交、SSE/队列进度、任务取消 / 重试 / 优先级、全站并发槽位、单用户并发槽位、多图拆分。
...
- **同步 vs 异步**：当前自托管版本统一创建异步任务并用 SSE / 任务列表推进度；未来若恢复单张小图同步路径，需要和队列配额、取消、重试语义保持一致。
...
- [x] 登录、注册、session、本地开发首个注册自动管理员、生产首个管理员 bootstrap token 保护、旧管理员初始化令牌兼容和注册防刷。
...
- [x] 完善备份 / 恢复脚本，覆盖 SQLite、WAL、用户图片和示例图。
...
- [ ] 把当前 SQLite 限流和单进程队列迁移到 Redis/BullMQ、SQS 或等价后端。
- [ ] 把 session 存储、限流事件、队列事件和后台维护任务拆出可横向扩展边界。
